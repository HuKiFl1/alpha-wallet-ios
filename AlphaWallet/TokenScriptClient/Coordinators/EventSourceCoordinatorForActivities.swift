...
class EventSourceCoordinatorForActivities {
    ...
    func fetchEthereumEventsImpl() {
        ...
        NSLog("xxx fetching events with promises: \(fetchPromises) count: \(fetchPromises.count)")
        //hhh restore
        //when(resolved: fetchPromises).done { _ in
        //hhh remove
        when(resolved: fetchPromises).done { results in
            NSLog("xxx done fetching events for Activities: \(results) count: \(results.count)")
    ...
    private func fetchEvents(token: TokenObject, eventOrigin: EventOrigin) -> Promise<Void> {
        ...
        //hhh have to start from 0 if the tokenscript file changes? Careful with problem with signature. But signature results aren't cached, right? Do they trigger as XML changed?
    ...
    private func convertEventToDatabaseObject(_ event: EventParserResultProtocol, filterParam: [(filter: [EventFilterable], textEquivalent: String)?], eventOrigin: EventOrigin, token: TokenObject, server: RPCServer) -> EventActivity? {
        ...
        return EventActivity(contract: eventOrigin.contract, tokenContract: token.contractAddress, server: server, eventName: eventOrigin.eventName, blockNumber: Int(blockNumber), logIndex: Int(logIndex), filter: filterText, json: json)
    }
